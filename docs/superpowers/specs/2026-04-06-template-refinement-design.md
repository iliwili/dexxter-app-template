...
**Frontend** — uses `@nuxt/eslint` module (official Nuxt 4 integration). Handles Vue files, auto-imports, and TypeScript automatically. Added to `nuxt.config.ts` modules. No additional custom rules — the Nuxt defaults are sufficient. An `eslint.config.mjs` is generated by `nuxt prepare`; no manual rule additions are needed.
...
2. Find-and-replace `app-name` with your project name in these exact files:
   - `backend/package.json`
   - `docker-compose.yml`
   - `.github/workflows/deploy.yml`
   - `README.md`
3. Update the VPS deploy path in `.github/workflows/deploy.yml` — the SSH script contains `cd ~/dexxter`, change `dexxter` to your project's directory name on the VPS
4. Copy `.env.example` files and fill in values: `cp .env.example .env`, `cp backend/.env.example backend/.env`, `cp frontend/.env.example frontend/.env`
5. Configure GitHub Actions secrets: `SSH_KEY`, `VPS_HOST`, `VPS_USER`, `TS_OAUTH_CLIENT_ID`, `TS_OAUTH_SECRET`
6. Push to `master` to trigger the first deploy
